...
RxBool isMyProduct=false.obs;
 ...
 Future<void> createProduct({
  ...
  try {
    ...
    if (response.statusCode == 200) {
      categories.value = response.data; // Map this response to your model if needed
...
Future<void> getAllProducts() async {
  isLoading.value = true;
  try {
    final response = await _apiClient.get(ApiEndpoints.allProducts); // Replace with actual endpoint
...
Future<void> sendProductRequest({
  ...
  try {
    final response = await _apiClient.post(
      ApiEndpoints.sendRequest, // Make sure this endpoint exists in ApiEndpoints
      data: {
...
Future<void> cancelRequest({
  ...
  try {
    final response = await _apiClient.patch(
      ApiEndpoints.cancelRequest, // Make sure this endpoint exists in ApiEndpoints
      data: {
...
Future<void> updateRequest({
  ...
  try {
    final response = await _apiClient.patch(
      ApiEndpoints.updateRequest, // Make sure this endpoint exists in ApiEndpoints
      data: {
        ApiEndpoints.requestId: requestId,
        ApiEndpoints.status:status
      },
...
isMyProduct.value=false;
  ...
  try {
    final response = await _apiClient.get(
      ApiEndpoints.productDetails+productId.toString(), // Make sure this endpoint exists in ApiEndpoints
...
Future<void> getRequestReceivedChart() async {
  ...
  try {
    final response = await _apiClient.get(
      ApiEndpoints.requestReceived, // Make sure this endpoint exists in ApiEndpoints
...
Future<void> getReceivedRequest() async {
  ...
  try {
    final response = await _apiClient.get(
      ApiEndpoints.receivedRequested, // Make sure this endpoint exists in ApiEndpoints
